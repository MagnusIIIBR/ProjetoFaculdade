    ...
    $.extend(neonChat, {
            ...
            $groups.each(function (i, el) {
                ...
                $contacts.each(function (i, el) {
                    ...
                    for (var i in neonChat.statuses) {
                        ...
                        else if ($status.hasClass(status.class)) {
                            $contact.data('order-id', status.order);
                            return true;
                        }
                    }
        ...
        showChat: function (animated) {
            ...
            if (animated) {
                ...
                TweenMax.set($chat, {
                    css: {
                        autoAlpha: public_vars.$body.hasClass('boxed-layout') ? 0 : 1,
                        transform: "translateX(" + ((public_vars.$pageContainer.hasClass('right-sidebar') ? -1 : 1) * chat_width) + "px)"
                    }
                });

                TweenMax.to($chat, .65, {
                    css: {autoAlpha: 1, transform: "translateX(0)"}, ease: Sine.easeOut, onComplete: function () {
                        public_vars.$pageContainer.addClass(visible_class);
